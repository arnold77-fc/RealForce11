(function () {
     ...
     * @param {function} oncomplite - функция успешного завершения
     * @param {function} onerror - функция ошибки
     */function list(){var params=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{};var oncomplite=arguments.length>1?arguments[1]:undefined;var onerror=arguments.length>2?arguments[2]:undefined;if(params.genres)return Lampa.Api.sources.tmdb.list(params,oncomplite,onerror);var u=url(params.id||params.url,params);network.silent(u,oncomplite,onerror);}function full(params,oncomplite){var require=1;// Количество обязательных запросов
    var status=new Lampa.Status(require);status.onComplite=oncomplite;var url=Manifest.api_host+'/catalog/card?plugin='+params.source+'&uri='+encodeURIComponent(params.id)+'&type='+(params.method||'movie');get(Utils.account(url),params,function(json){// Источник
     ...
     * @param {function} oncomplite - функция успешного завершения
     * @returns {void}
     */function search(catalog_name){var params=arguments.length>1&&arguments[1]!==undefined?arguments[1]:{};var oncomplite=arguments.length>2?arguments[2]:undefined;var catalog=Manifest.catalogs[catalog_name];if(!catalog)return oncomplite([]);var u=Utils.account(catalog.search);if(!u)return oncomplite([]);get(u,params,function(json){json.title='Найдено';oncomplite([json]);},function(){oncomplite([]);});}function person(){var params=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{};var oncomplite=arguments.length>1?arguments[1]:undefined;var onerror=arguments.length>2?arguments[2]:undefined;Lampa.Api.sources.tmdb.person(params,oncomplite,onerror);}/**
    ...
    function loadCatalogs(){var network=new Lampa.Reguest();network.silent(Utils.account(Manifest.api_host+'/catalog'),function(json){var keys=Object.keys(json);Manifest.catalogs=json;keys.forEach(function(key){Lampa.Params.values.source[key]=key;Object.defineProperty(Lampa.Api.sources,key,{get:function get(){return Api;}});Lampa.Search.addSource(Api.discovery(key));});});Lampa.Listener.follow('menu',function(e){if(e.type=='action'&&(e.action=='cartoon'||e.action=='anime')){var catalog=Api.getCatalog();if(!catalog)return;if(e.action=='cartoon'&&catalog.cartoons){Lampa.Router.call('category',{url:'cartoons',title:Lampa.Lang.translate('menu_multmovie')+' - '+Lampa.Storage.field('source').toUpperCase()});e.abort();}if(e.action=='anime'&&catalog.anime){Lampa.Router.call('category',{url:'anime',title:Lampa.Lang.translate('menu_anime')+' - '+Lampa.Storage.field('source').toUpperCase()});e.abort();}}});}function startPlugin(){window.plugin_catalog=true;if(window.appready)loadCatalogs();else {Lampa.Listener.follow('app',function(e){if(e.type=='ready')loadCatalogs();});}}if(!window.plugin_catalog)startPlugin();
